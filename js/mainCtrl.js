angular.module('itunes').controller('mainCtrl', function($scope, itunesService){
  ...
  */
  //the iTunes API is going to give you a lot more details than ng-grid wants. Create a new array and then loop through the iTunes data pushing into your new array objects that look like the above data.
    //
    // $scope.gridFormat = function(dataArray) {
    //   var gridArray = [];
    //   for (var i = 0; i < dataArray.length; i++) {
    //     if (dataArray.data.results[i] === 'AlbumArt') {
    //       gridArray.push(dataArray.data.results[i]);
    //     };
    //     if (dataArray.data.results[i] === 'Artist') {
    //       gridArray.push(dataArray.data.results[i]);
    //     };
    //     if (dataArray.data.results[i] === 'Collection') {
    //       gridArray.push(dataArray.data.results[i]);
    //     };
    //     if (dataArray.data.results[i] === 'CollectionPrice') {
    //       gridArray.push(dataArray.data.results[i]);
    //     };
    //     if (dataArray.data.results[i] === 'Play') {
    //       gridArray.push(dataArray.data.results[i]);
    //     };
    //     if (dataArray.data.results[i] === 'Type') {
    //       gridArray.push(dataArray.data.results[i]);
    //     };
    //   };
    // }

    // $scope.gridOptions = $scope.gridFormat(response);
